...
```{r}
flux_sol <- read_csv(here("input", "perturbed_flux_trajectories.csv"))
```
...
```{r}
concentration_trajectories = read_csv(here("input", "perturbed_concentration_trajectories.csv"))
```
...
p1 <- concentration_long %>%
  ...
  labs(
    x = "time [hr]",
    y = "concentration [mM]",
    title = "Concentrations, HGPRT 10%"
  )

ggsave(here("output", "perturbed_HGPRT_activity_010.png"), plot = p1, units = "in", dpi = 300, height = 3, width = 5)
