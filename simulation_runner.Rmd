...
```{r}
cl <- makeCluster(num_cores, type = "PSOCK") # 7596.613 ms on Threadripper, 56 workers
...
clusterEvalQ(cl, {
  ...
  library(ggplot2)
  source("simulation_core.R")
}) # 1910.141 ms on Threadripper, 56 workers

clusterSetRNGStream(cl, iseed = 1234)  # 4.1063	ms on Threadripper, 56 workers
...
# Above took 176946.2 ms on Threadripper, 56 workers, 576 tasks 
# (note processor usage only spikes for a few seconds), 307 ms/task

stopCluster(cl)  # 4.1195	ms on Threadripper, 56 workers
```
